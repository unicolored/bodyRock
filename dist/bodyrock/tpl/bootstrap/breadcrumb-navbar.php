...
<ol class="breadcrumb">
  ...
  <?php if(is_single()) {
                            ...
                            $output .= '<li><a href="'.get_category_link( $category->term_id ).'" title="' . esc_attr( sprintf( __( "View all posts in %s",'bodyrock' ), $category->name ) ) . '">'.ucfirst(str_replace('<br>','',$category->cat_name)).'</a></li>';
                            }
